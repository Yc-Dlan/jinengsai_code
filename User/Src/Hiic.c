...
{
	return HAL_I2C_Mem_Read(&hi2c1,Salve_Adress,Reg_Address,I2C_MEMADD_SIZE_8BIT,Result,len,10000)==HAL_OK;
}
unsigned char IIC_ReadBytes2(unsigned char Salve_Adress,unsigned char Reg_Address,unsigned char *Result,unsigned char len)
{
	return HAL_I2C_Mem_Read(&hi2c2,Salve_Adress,Reg_Address,I2C_MEMADD_SIZE_8BIT,Result,len,10000)==HAL_OK;
}
unsigned char IIC_ReadBytes3(unsigned char Salve_Adress,unsigned char Reg_Address,unsigned char *Result,unsigned char len)
{
	return HAL_I2C_Mem_Read(&hi2c3,Salve_Adress,Reg_Address,I2C_MEMADD_SIZE_8BIT,Result,len,10000)==HAL_OK;
}
//unsigned char IIC_ReadBytes3(unsigned char Salve_Adress,unsigned char Reg_Address,unsigned char *Result,unsigned char len)
//{
//	return HAL_I2C_Mem_Read(&hi2c3,Salve_Adress,Reg_Address,I2C_MEMADD_SIZE_8BIT,Result,len,10000)==HAL_OK;
...
{
	unsigned char dat;
	IIC_ReadBytes(GW_GRAY_ADDR_DEF1<<1,GW_GRAY_PING,&dat,1);
...
{
	unsigned char dat;
	IIC_ReadBytes2(GW_GRAY_ADDR_DEF2<<1,GW_GRAY_PING,&dat,1);
	if(dat==GW_GRAY_PING_OK)
	{
			return 0;
	}	
...
{
	unsigned char dat;
	IIC_ReadBytes3(GW_GRAY_ADDR_DEF2<<1,GW_GRAY_PING,&dat,1);
	if(dat==GW_GRAY_PING_OK)
	{
			return 0;
	}	
...
{
	unsigned char dat;
	IIC_ReadBytes(GW_GRAY_ADDR_DEF1<<1,GW_GRAY_DIGITAL_MODE,&dat,1);
	return dat;
}
unsigned char IIC_Get_Digtal2(void)
{
	unsigned char dat;
	IIC_ReadBytes2(GW_GRAY_ADDR_DEF2<<1,GW_GRAY_DIGITAL_MODE,&dat,1);
	return dat;
}
unsigned char IIC_Get_Digtal3(void)
{
	unsigned char dat;
	IIC_ReadBytes3(GW_GRAY_ADDR_DEF2<<1,GW_GRAY_DIGITAL_MODE,&dat,1);
	return dat;
}
unsigned char IIC_Get_Anolog(unsigned char * Result,unsigned char len)
{
	if(IIC_ReadBytes(GW_GRAY_ADDR_DEF1<<1,GW_GRAY_ANALOG_BASE_,Result,len))return 1;
	else return 0;
}
unsigned char IIC_Get_Single_Anolog(unsigned char Channel)
{
	unsigned char dat;
	IIC_ReadBytes(GW_GRAY_ADDR_DEF1<<1,GW_GRAY_ANALOG(Channel),&dat,1);
	return dat;
}
unsigned char IIC_Anolog_Normalize(uint8_t Normalize_channel)
{
	return IIC_WriteBytes(GW_GRAY_ADDR_DEF1<<1,0xCF,&Normalize_channel,1);
}
unsigned short IIC_Get_Offset(void )
{
	unsigned char dat[2]={0};
	IIC_ReadBytes(GW_GRAY_ADDR_DEF1<<1,Offset,dat,2);
...
//{
//	if(IIC_ReadBytes3(Color_Adress<<1,HSL_Reg,Result,len))return 1;
//	else return 0;
//}
